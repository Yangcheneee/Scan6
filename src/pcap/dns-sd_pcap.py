...
def handle_mdns_packet(packet):
    try:
        if packet.haslayer(DNS) and packet.haslayer(IP):
            ...
            if dns.qr == 1 and packet[IP].dst=="224.0.0.251":  # QR=1表示响应
                info = {
                    "mac": None,
                    "ip4": None,
                    "hostname": None,
...
def run(pcap_path='data/mdns.pcapng', save_file="result/dns-sd.csv"):
    mdns_info_list = []
...
if __name__ == "__main__":
    run("data/mdns.pcapng", "result/dns-sd.csv")
